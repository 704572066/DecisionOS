...
function App(){
 ...
 const load=async()=>{const r=await fetch(`${API}/api/projects`); const p=await r.json(); setProjects(p); if(p[0])setProjectId(p[0].id)};
 useEffect(()=>{load()},[]);
 const seed=async()=>{const r=await fetch(`${API}/api/demo/seed`,{method:'POST'}); const d=await r.json(); setMessage(d.message); await load(); setProjectId(d.projectId)};
 const createMeeting=async()=>{const r=await fetch(`${API}/api/meetings`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({projectId,title:'客户商务谈判'})}); const d=await r.json(); setMeetingId(d.id); setMessage(`会议已创建：${d.id}`)};
 const analyze=async()=>{if(!meetingId)return setMessage('请先创建会议'); await fetch(`${API}/api/meetings/${meetingId}/transcript`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})}); const r=await fetch(`${API}/api/meetings/${meetingId}/analyze`,{method:'POST'}); const d=await r.json(); setReminders(d.reminders); setMessage(`识别主题：${(d.topics||[]).join('、')||'暂无'}`)};
 const save=async()=>{const r=await fetch(`${API}/api/decisions`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({projectId,meetingId:meetingId||null,title:'客户A价格与付款条件决策',statement:decision,evidenceSummary:reminders.map(x=>x.summary).join('\n'),taskTitle:'准备修订报价方案',taskObjective:'形成折扣与付款周期组合方案',taskOwner:'销售负责人'})}); const d=await r.json(); setMessage(`已保存 Decision ${d.decisionId}，Task ${d.taskId||'未创建'}`)};
